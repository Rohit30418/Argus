...
const scanRateLimitPerHour = Math.max(0, Number(process.env.ARGUS_SCAN_RATE_LIMIT_PER_HOUR || 0));
const scanHits = new Map();

function allowScanFrom(ip) {
  if (!scanRateLimitPerHour) return true;
  const now = Date.now();
  const cutoff = now - 60 * 60 * 1000;
  const recent = (scanHits.get(ip) || []).filter(ts => ts > cutoff);
  if (recent.length >= scanRateLimitPerHour) {
...
app.get('/healthz', (req, res) => res.status(200).json({ ok:true, service:'ARGUS', version:'3.0.0' }));
...
app.post('/api/scans', async (req,res)=>{
  if(!allowScanFrom(req.ip || req.socket?.remoteAddress || 'unknown')) return res.status(429).json({error:'Scan rate limit reached. Please try again later.'});
...
app.listen(PORT, '0.0.0.0',()=>console.log(`ARGUS Pro v3 running on 0.0.0.0:${PORT}`));
